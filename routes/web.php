...
// === IDINAGDAG: mula sa file ng kaklase ===
use App\Http\Controllers\PaymentVerificationController;
use App\Http\Controllers\XenditWebhookController;
use App\Http\Controllers\User\XenditPaymentController as UserXenditPaymentController;
// === END IDINAGDAG ===
...
Route::prefix('app')
    ...
    ->group(function () {
        ...
        // === IDINAGDAG: Xendit GCash payment (mula sa kaklase) ===
        Route::get('/bookings/{booking}/pay-gcash', [UserXenditPaymentController::class, 'pay'])->name('bookings.pay-gcash');
        Route::get('/bookings/{booking}/pay-gcash/success', [UserXenditPaymentController::class, 'success'])->name('bookings.pay-gcash.success');
        Route::get('/bookings/{booking}/pay-gcash/failure', [UserXenditPaymentController::class, 'failure'])->name('bookings.pay-gcash.failure');
        // === END IDINAGDAG ===
    });

// === IDINAGDAG: Xendit webhook (mula sa kaklase) ===
// publicly accessible, no auth/session (Xendit calls this server-to-server).
// CSRF dapat i-exclude sa path na ito sa bootstrap/app.php.
...
// === IDINAGDAG: mula sa kaklase ===
Route::get('/pos/lookup-booking', [PosController::class, 'lookupBooking']);
// === END IDINAGDAG ===
...
// === IDINAGDAG: mula sa kaklase (payment verification para sa manual QR Ph upload) ===
Route::get('/payment-verification', [PaymentVerificationController::class, 'index'])->name('payment-verification');
Route::post('/payment-verification/{booking}/approve', [PaymentVerificationController::class, 'approve'])->name('payment-verification.approve');
Route::post('/payment-verification/{booking}/reject', [PaymentVerificationController::class, 'reject'])->name('payment-verification.reject');
// === END IDINAGDAG ===
